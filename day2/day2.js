...
const reports = (0, tooling_1.initialDataParse)("data.txt", "lines");
console.log(reports);
let safe = 0;
let dampSafe = 0;
let unsafe = 0;
let dampUnsafe = 0;
function isSafeReading(reading) {
    if (reading[1] > reading[0]) {
        return reading.every((e, i, a) => i === 0 || (a[i] > a[i - 1] && a[i] <= a[i - 1] + 3));
    }
...
function damper(dampReading, backstep = false) {
    let i = 1;
    if (dampReading[1] > dampReading[0]) {
        while (i < dampReading.length) {
            if (!(dampReading[i] > dampReading[i - 1] &&
...
function splicer(dampReading, failure, backstep) {
    if (backstep) {
